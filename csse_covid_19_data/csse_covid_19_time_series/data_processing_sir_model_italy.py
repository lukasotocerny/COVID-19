    ...
    with open('time_series_19-covid-{}.csv'.format(dataset), 'r') as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                continue
            if row[1] == country:
                for i in range(30, end_day):
                    res[i-start_day] += int(row[i])
    return np.array(res)

confirmed_china = process_dataset("Confirmed", "Italy")
deaths_china = process_dataset("Deaths", "Italy")
recovered_china = process_dataset("Recovered", "Italy")
